...
void createWorld(Scene &world)
{
    ...
    auto lightMat = make_shared<Light>(vec3(10.0f));
    ...
    //Currently transform can't apply to trimesh, for its every triangles' bounding box doesn't transform.
    // world.AddObject(make_shared<Instance>(tfmRight, square, lightMat));
    ...
    // world.AddObject(make_shared<Instance>(tfmLeft, square, lightMat));
    for (auto a : square->GetTriangles())
    {
    ...
    // world.AddObject(make_shared<Instance>(tfmBottom, square, lightMat));
    for (auto a : square->GetTriangles())
    {
    ...
    // world.AddObject(make_shared<Instance>(tfmTop, square, lightMat));
     for (auto a : square->GetTriangles())
    {
    ...
    mat4 tfmLight(1.0f);
    tfmLight = glm::translate(tfmLight, vec3(0, 2.98, 0));
